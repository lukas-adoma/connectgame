...
def alphabeta(board, depth, alpha, beta, maximizingPlayer):
    ...
    if maximizingPlayer:
        ...
        for col in valid_locations:
            ...
            new_score = max(value, alphabeta(b_copy, depth - 1, alpha, beta, False)[1])
            if new_score > value:
                value = new_score
                column = col
        ...
        for col in valid_locations:
            ...
            new_score = min(value, alphabeta(b_copy, depth - 1, alpha, beta, True)[1])
            if new_score < value:
                value = new_score
                column = col
            beta = min(beta, value)
...
while not game_over:
    for event in pygame.event.get():
        ...
        if turn == AI and not game_over:
            ...
            col, minimax_score = alphabeta(board, 4, -math.inf, math.inf, True)
            if is_valid_location(board, col):
                row = get_next_open_row(board, col)
